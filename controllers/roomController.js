  ...
  con.query(sqlcheck, [roomName], function (err, data, fields) {
    if (data.length > 0) {
      ...
      if (status.toLowerCase() === 'available') {
        ...
        var update =
          'update rooms set occupancyString = ? where roomName = ?';
        con.query(
          update,
                      ...
                      function (err, data, fields) {
                        if (data) {
                          if (starttime > endtime) {
                            ...
                            res.render("rooms", {
                              error: "Start Date and End Date should not be different!",
                            });
                          } else if (starttime == endtime) {
                            res.render("rooms", {
                              error: "Start time and end time should not be equal!",
                            });
                          }
                          else if (people.length - 1 === index) {
                            con.query(
                              "Insert into meetingSchedule(roomname,title,startdate,enddate,starttime,endtime) values('" +
                              roomName +
                              "','" +
                              title +
                              "','" +
